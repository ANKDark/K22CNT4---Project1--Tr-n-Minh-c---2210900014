...
$sql_dh_tmd = "SELECT * FROM donhang LIMIT $offset, $limit_tmd";
$result_tmd = $conn_tmd->query($sql_dh_tmd);
...
if (isset($_GET['srch_tmd'])) {
    ...
    $sql_search = "SELECT * FROM donhang WHERE SPID_TMD LIKE '%$searchTerm%'";
    ...
    if ($result_search->num_rows > 0) {
        while ($row_search = $result_search->fetch_assoc()) {
            ...
            echo "<td>{$row_search['MADONHANG']}</td>";
            ...
            echo "<td>{$row_search['TINH_TMD']}</td>";
            echo "<td>{$row_search['HUYEN_TMD']}</td>";
            echo "<td>{$row_search['HINHTHUCTHANHTOAN']}</td>";
            echo "<td>{$row_search['NGAYDAT_TMD']}</td>";
            echo "<td>";
            echo "<button class='btn btn-primary btn-sm trash' type='button' title='Xóa' onclick=\"button_Delete_tmd('{$row_search['MADONHANG']}')\"><i class='ank_fa_tmd bx bx-trash'></i></button>";
            echo "</td>";
            echo "</tr>";
        }
    } else {
...
<body class="dark">
    ...
    <div class="sidebar_tmd">
        ...
        <ul class="app_menu_tmd">
            ...
            <li><a class="app_menu_item_tmd active" href="quanly_donhang_tmd.php"><i class='fa bx fa bx-task'></i><span
    ...
    <div class="app_content_tmd">
        <div class="app_title_tmd">
            <ul class="app_breadcrumb_tmd breadcrumb_tmd">
                <li class="breadcrumb_item_tmd"><a href="#">Danh sách đơn hàng</a></li>
            </ul>
                                ...
                                if ($result_tmd->num_rows > 0) {
                                    while ($row = $result_tmd->fetch_assoc()) {
                                        ...
                                        <tr class="product-row">
                                            ...
                                            <td>
                                                <?php echo $row['TINH_TMD']; ?>
                                            </td>
                                            <td>
                                                <?php echo $row['HUYEN_TMD']; ?>
                                            </td>
                                            <td>
                                                <?php echo $row['TONGTIEN_TMD']; ?>
                                            </td>
                                            <td>
                                                <?php echo $row['NGAYDAT_TMD']; ?>
                                            </td>
                                            <td>
                                                <?php echo $row['HINHTHUCTHANHTOAN'] == 1 ? "Thanh toán chuyển khoản" : "Thanh toán khi nhận hàng"; ?>
                                            </td>
                                            <td>
                                                <button class="btn btn-primary btn-sm trash" type="button" title="Xóa"
                                                    onclick="button_Delete_tmd('<?php echo $row['MADONHANG']; ?>')"><i
                                                        class='ank_fa_tmd bx bx-trash'></i></i>
                                                </button>
                                                <button class="btn btn-primary btn-sm edit" type="button" title="Sửa"
                                                    id="show-emp" data-toggle="modal" data-target="#ModalUP"
                                                    onclick="button_Edit_tmd('<?php echo $row['MADONHANG']; ?>')">
                                    ...
                                    ?>
                                    <tr>
                                        <td colspan="13" style="  text-align: center;">Chưa có đơn hàng nào</td>
                                    </tr>
                                    <?php
                                }
                                ?>
        ...
        <div class="swal-overlay_tmd swal-overlay--show-modal" tabindex="-1">
            <div class="swal-modal" role="dialog" aria-modal="true">
                ...
                <div class="swal-text">Bạn có chắc chắn là muốn xóa đơn hàng này?</div>
...
<script>
    function button_Delete_tmd(MADONHANG) {
        ...
        confirmButton.addEventListener('click', function () {
            ...
            xhr.open("POST", "../BackendPHP/xoa_donhang_tmd.php", true);
